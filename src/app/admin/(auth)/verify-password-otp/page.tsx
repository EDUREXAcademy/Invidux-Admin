...
import { useVerifyPasswordOtp, useResendPasswordOtp } from "@/hooks/mutations";
...
const VerifyPasswordOtp = () => {
  const {mutate, isPending} = useVerifyPasswordOtp();
  const {mutate:resendMutate, isPending:resendPending} = useResendPasswordOtp();
  ...
  return (
    <div className="px-[5%] flex justify-center items-center gap-x-16 py-10 md:py-6">
      ...
      <Slider/>
      <div>
        <Link href='/token-issuers/login'><Image src={logo} alt="Invidux logo" className='w-[100px] block md:hidden mb-6'/></Link>
        ...
        <form onSubmit={handleSubmit(verifyUserEmail)} className="mb-2 md:mb-[32px]">
        ...
        <div className="flex gap-x-[9px] mb-[24px]">
          ...
          <p className="text-[#B1924E] text-base font-normal underline leading-normal cursor-pointer"
            onClick={(e) => {
              e.preventDefault();
